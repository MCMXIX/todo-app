    ...
    <div class="m-auto rounded bg-cyan-900 drop-shadow-lg w-1/2">
        <div class="flex flex-col text-center text-gray-100 p-5">
            ...
            <div class="flex flex-col pt-10">
                <button class="w-28 text-center rounded h-10 mx-auto bg-slate-400 text-black hover:font-bold border-2 border-white" @click="createNewUser">SUBMIT</button>
            </div>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods : {
            ...
            async createNewUser() {
                this.validateFields();
                if (this.aErrorFields.length < 1 && this.bIsProcessing === false) {
                    this.bIsProcessing = true;
                    await this.createUser(this.oUserInfo);
