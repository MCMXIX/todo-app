<template>
    <div class="container h-14 mx-auto pt-4 pr-2 w-2/3">
        <div class="w-full flex flex-row">
            <div class="w-1/2">
                <button :class="{'activeNavItem' : getIsActive('home'), 'navItem' : !getIsActive('home') }" @click="$router.push({ path : '/' })"> HOME </button>
            </div>
...
<style scoped>
    .navItem {
        @apply text-xl first-letter:font-normal text-white p-1 hover:font-bold;
    }
    .activeNavItem {
        @apply text-xl font-bold text-white p-1;
    }
</style>